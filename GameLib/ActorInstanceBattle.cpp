...
bool IS_CANNOT_PUSH_RACE(unsigned int vnum)
{
	switch (vnum)
	{
		case 768:
...
bool CActorInstance::__CanPushDestActor(CActorInstance& rkActorDst)
{
	...
	if (IS_CANNOT_PUSH_RACE(rkActorDst.GetRace())) {
		return false;
	}
#endif
...
void CActorInstance::__ProcessDataAttackSuccess(const NRaceData::TAttackData & c_rAttackData, CActorInstance & rVictim, const D3DXVECTOR3 & c_rv3Position, UINT uiSkill, BOOL isSendPacket)
{
	...
	if (uiSkill) {
		rVictim.m_fInvisibleTime = CTimer::Instance().GetCurrentSecond() + 
#ifdef ENABLE_BUGFIXES
		(c_rAttackData.fInvisibleTime - __GetInvisibleTimeAdjust(uiSkill, c_rAttackData))
#else
