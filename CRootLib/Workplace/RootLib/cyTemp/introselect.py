...
class SelectCharacterWindow(ui.Window) :	
	...
	if app.ENABLE_WOLFMAN_CHARACTER:
		...
		DISC_FACE_PATH.update({playersettingmodule.RACE_WOLFMAN_M : "d:/ymir work/bin/icon/face/wolfman_m.tga",})
	...
	if app.ENABLE_PLAYER_PIN_SYSTEM:
		...
		def AcceptInputPinCode(self):
			...
			id = self.GetCharacterSlotID(self.RealSlot[self.SelectSlot])
			if 0 == id:
