...
class AttachMetinDialog(ui.ScriptWindow):
	...
	def __LoadScript(self):
		...
		newToolTip.SetPosition(230 + 65, 38)
	...
	def Open(self, metinItemPos, targetItemPos):
		...
		if app.ENABLE_EXTRA_INVENTORY:
			metinIndex = player.GetItemIndex(player.EXTRA_INVENTORY, metinItemPos)
		else:
	...
	def UpdateDialog(self):
		newWidth = self.newToolTip.GetWidth() + 230 + 80
	...
	def Accept(self):
		if app.ENABLE_EXTRA_INVENTORY:
			net.SendItemUseToItemPacket(player.EXTRA_INVENTORY, self.metinItemPos, player.INVENTORY, self.targetItemPos)
		else:
