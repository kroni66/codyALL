...
class SwitchbotItemTab(ui.ThinBoardCircle):
	...
	def OverInItem(self, overSlotPos):
		...
		if app.ATTR_LOCK:
			lockedattr = player.GetItemAttrLocked(player.SWITCHBOT, self.slot_num)
			
		self.tooltipItem.AddItemData(itemVnum, metinSlot, attrSlot, 0, 0, player.SWITCHBOT, self.slot_num, lockedattr)	
