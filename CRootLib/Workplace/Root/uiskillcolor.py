...
class SkillColorWindow(ui.ScriptWindow):
	...
	def __BindObject(self):
		...
		self.remainUse = self.GetChild("remainUse")
	...
	def UpdateUseAvailable(self):
		...
		for i in xrange(m):
			...
			if itemVnum == 164406:
				c += player.GetItemCount(i)
		
		self.remainUse.SetText(localeinfo.SKILL_CHANGE_AVAILABLE % (c))
	...
	def GetSavedColorPresets(self, skillSlot):
		try:
			lines = open(HEX_COLOR_FILE, "r").readlines()
			hexColorList = []
			for line in lines:
				tokens = line[:-1].split("\t")
				...
				slot = int(tokens[0])
				if slot == skillSlot:
					for hexPos in xrange(1, len(tokens)):
						hexColorList.append(tokens[hexPos])
			
			return hexColorList
		except:
			file = old_open(HEX_COLOR_FILE, "w")
			file.close()
			self.GetSavedColorPresets(skillSlot)
	...
	def OnClickSaveColorButton(self):
		...
		if hexCode == "" or hexCode == "#":
			self.popup.SetText(localeinfo.SKILL_COLOR_SELECT_FIRST)
			self.popup.Open()
			return
		
		hasSlot = False
		with open(HEX_COLOR_FILE, "r+") as file:
			lines = file.readlines()
			if len(lines) == 0:
				file.write("%d\t%s" % (self.skillSlot, hexCode))
				return
			...
			for i, line in enumerate(lines):
				...
				slot = int(tokens[0])
				if slot != self.skillSlot:
					continue
				
				if len(tokens) - 1 >= COLOR_PRESET_SELECT_UI_SHOW_MAX:
					return
	...
	def OnClickClearColorButton(self):
		...
		fin = open(HEX_COLOR_FILE)
		fout = open(HEX_COLOR_FILE, "w+")
		for line in fin:
			tokens = line[:-1].split("\t")
			if len(tokens) > 0 and tokens[0] and not tokens[0] == "#":
				slot = int(tokens[0])
				if slot == self.skillSlot:
					line = line.replace("\t" + selectedColorCode, "")
			fout.write(line)
	...
	def __CreateColorPresetButton(self):
		...
		for i in xrange(min(COLOR_PRESET_SELECT_UI_SHOW_MAX, dictLen)):
			...
			button.SetEvent(ui.__mem_func__(self.OnClickColorPreset), i)
			button.SetOverEvent(ui.__mem_func__(self.OnColorPresetOver), i)
			button.SetOverOutEvent(ui.__mem_func__(self.OnColorPresetOut), i)
			button.SetListText(colorPresetList[i]) # [:HEX_CODE_LENGTH]
			button.Hide()
