...
#endif
				{
					int type = htoi(results[i].c_str());
					if (type != 0 && !isAttr) {
						isAttr = true;
					}

					len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x:%lld", type, atoi(results[i+1].c_str()));
				}
			}
...
void initChat()
{
	...
	PyModule_AddIntConstant(poModule, "CHAT_SET_LOG_WINDOW", 0);
	PyModule_AddIntConstant(poModule, "CHAT_NEW_WINDOW_START_IDX", 1);
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_INFO_EXP", CHAT_TYPE_INFO_EXP);
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_INFO_ITEM", CHAT_TYPE_INFO_ITEM);
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_INFO_VALUE", CHAT_TYPE_INFO_VALUE);
#else
