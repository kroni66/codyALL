...
PyObject* netSendWhisperAdminPacket(PyObject* poSelf, PyObject* poArgs)
{
	char* szText;
	...
	char* szLang;
	if (!PyTuple_GetString(poArgs, 1, &szLang))
		return Py_BuildException();

	int color;
	if (!PyTuple_GetInteger(poArgs, 2, &color))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendWhisperAdminPacket(szText, szLang, color);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendItemMovePacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendItemMovePacket(Cell, ChangeCell, 
...
PyObject* netSendShopBuyMultiplePacket(PyObject* poSelf, PyObject* poArgs) {
	uint8_t p;
	if (!PyTuple_GetByte(poArgs, 0, &p)) {
		return Py_BuildException();
	}
	...
	CPythonNetworkStream::Instance().SendShopBuyMultiplePacket(p, c);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendShopSellPacketNew(PyObject* poSelf, PyObject* poArgs)
{
	...
	int window = 0;
	if (!PyTuple_GetInteger(poArgs, 2, &window))
		return Py_BuildException();

	rkNetStream.SendShopSellPacketNew(TItemPos(window, iSlotNumber), iCount);
#else
...
PyObject* netRequestTargetLang(PyObject* poSelf, PyObject* poArgs)
{
	char* targetName;
	if (!PyTuple_GetString(poArgs, 0, &targetName))
		return Py_BuildException();
	
	CPythonNetworkStream::Instance().SendRequestTargetLang(targetName);
	return Py_BuildNone();
}
#endif

#ifdef ENABLE_MAP_TELEPORTER
PyObject* netSendMapTeleporterPacket(PyObject* poSelf, PyObject* poArgs)
{
	int iMapCode = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &iMapCode))
		return Py_BuildException();

	CPythonNetworkStream::instance().SendMapTeleporterPacket(iMapCode);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendItemDivisionPacket(PyObject* poSelf, PyObject* poArgs)
{
	TItemPos pos;
	if (!PyTuple_GetInteger(poArgs, 0, &pos.cell))
		return Py_BuildException();
	
	if (!PyTuple_GetInteger(poArgs, 1, &pos.window_type))
		return Py_BuildException();
	
	CPythonNetworkStream::Instance().SendItemDivisionPacket(pos);
	return Py_BuildNone();
}
...
PyObject* netSendOpenShopPacket(PyObject* poSelf, PyObject* poArgs)
{
	int32_t shopid;
	if (!PyTuple_GetInteger(poArgs, 0, &shopid)) {
		return Py_BuildException();
	}

	CPythonNetworkStream::Instance().SendOpenShopPacket(shopid);
	return Py_BuildNone();
}
#endif
...
void initnet()
{
	...
	{
#ifdef ENABLE_MAP_TELEPORTER
		{ "SendMapTeleporterPacket",				netSendMapTeleporterPacket,					METH_VARARGS },
#endif
#ifdef ENABLE_SEND_TARGET_INFO
...
#ifdef __ENABLE_EXTEND_INVEN_SYSTEM__
		{ "Envanter_expansion",					netEnvanterpaketi,						METH_VARARGS },
#endif
...
#ifdef ENABLE_BUY_STACK_FROM_SHOP
		{"SendShopBuyMultiplePacket", netSendShopBuyMultiplePacket, METH_VARARGS},
#endif
		...
		{"RequestTargetLang", netRequestTargetLang, METH_VARARGS},
#endif
#ifdef ENABLE_WHISPER_ADMIN_SYSTEM
		{ "SendWhisperAdminPacket",					netSendWhisperAdminPacket,					METH_VARARGS },
#endif
		{"SendItemDivisionPacket", netSendItemDivisionPacket, METH_VARARGS},
#ifdef ENABLE_NEW_FISHING_SYSTEM
...
#endif
#ifdef ENABLE_OPENSHOP_PACKET
		{"SendOpenShopPacket", netSendOpenShopPacket, METH_VARARGS},
#endif
#if defined(ENABLE_CHRISTMAS_WHEEL_OF_DESTINY)
...
#if defined(PYTHON_DYNAMIC_MODULE_NAME)
	PyObject* poModule = Py_InitModule(GetModuleName(NET_MODULE).c_str(), s_methods);
#else
