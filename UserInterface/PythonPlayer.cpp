...
WORD CPythonPlayer::GetItemCountbyVnumExtraInventory(DWORD dwVnum)
{
	DWORD dwCount = 0;

	for (int i = 0; i < c_Extra_Inventory_Refine; ++i)
	{
		const TItemData & c_rItemData = m_playerStatus.aItem[i];
		if (c_rItemData.vnum == dwVnum)
		{
...
void CPythonPlayer::SetItemCount(TItemPos Cell,
...
void CPythonPlayer::RequestAddLocalQuickSlot(DWORD dwLocalSlotIndex, DWORD dwWndType, DWORD dwWndItemPos)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendQuickSlotAddPacket((BYTE)dwGlobalSlotIndex, (BYTE)dwWndType, (WORD)dwWndItemPos);
}
...
void CPythonPlayer::RequestUseLocalQuickSlot(DWORD dwLocalSlotIndex)
{
	...
	switch (dwRegisteredType)
	{
		...
		{
			CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
			rkNetStream.SendItemUsePacket(TItemPos(EXTRA_INVENTORY, (WORD)dwRegisteredItemPos));
			break;
		}
#endif
...
float CPythonPlayer::GetSkillNextEfficientPercentageByLvl(DWORD dwSlotIndex, DWORD dwLevel)
{
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return 0;
	
	return LocaleService_GetSkillPower(dwLevel)/100.0f;
}
#endif
...
CPythonPlayer::CPythonPlayer(void)
{
	...
	static constexpr int EventsPerSecond = 4; //4 volte al second - si pu?ridurre anche ad 1 volendo
	...
	static Event::Event<>::EventFunc EventFunc = [](const auto& Info/*Unused Info*/) {
		if (CPythonSystem::Instance().GetPickUpMode()) {
