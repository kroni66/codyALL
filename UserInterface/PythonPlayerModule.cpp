...
PyObject * playerGetSkillNextEfficientPercentageByLvl(PyObject* poSelf, PyObject* poArgs)
{
	int iSlotIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSlotIndex))
		return Py_BuildException();
	...
	return Py_BuildValue("f", CPythonPlayer::Instance().GetSkillNextEfficientPercentageByLvl(iSlotIndex, iLevel));
}
#endif
...
PyObject * playerGetItemLink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		len = snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%x:%x:%x:[{%d}]", pPlayerItem->vnum, pPlayerItem->flags, pPlayerItem->alSockets[0], pPlayerItem->alSockets[1], pPlayerItem->alSockets[2], pPlayerItem->vnum);
#else
		...
		{
			if (pPlayerItem->aAttr[i].bType != 0 && !isAttr) {
				isAttr = true;
			}
...
void initPlayer()
{
...
#ifdef ENABLE_NEW_PASSIVE_SKILLS
		{"GetSkillNextEfficientPercentageByLvl", playerGetSkillNextEfficientPercentageByLvl, METH_VARARGS},
#endif
...
#if defined(PYTHON_DYNAMIC_MODULE_NAME)
	PyObject* poModule = Py_InitModule(GetModuleName(PLAYER_MODULE).c_str(), s_methods);
#else
	PyObject* poModule = Py_InitModule("player", s_methods);
#endif
#ifdef ENABLE_LOCKED_EXTRA_INVENTORY
	PyModule_AddIntConstant(poModule, "EXTRA_INVENTORY1", POINT_EXTRA_INVENTORY1);
	PyModule_AddIntConstant(poModule, "EXTRA_INVENTORY2", POINT_EXTRA_INVENTORY2);
	PyModule_AddIntConstant(poModule, "EXTRA_INVENTORY3", POINT_EXTRA_INVENTORY3);
	PyModule_AddIntConstant(poModule, "EXTRA_INVENTORY4", POINT_EXTRA_INVENTORY4);
	PyModule_AddIntConstant(poModule, "EXTRA_INVENTORY5", POINT_EXTRA_INVENTORY5);
#endif
	...
	PyModule_AddIntConstant(poModule, "RUNE_SLOT_START", c_Wear_Rune_Start);
	PyModule_AddIntConstant(poModule, "RUNE_SLOT_COUNT", c_Wear_Rune_Count);
#endif
